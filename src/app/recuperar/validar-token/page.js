...
export default function ValidarToken() {
  ...
  const [step, setStep] = useState(1); // 1: email + token, 2: nueva contraseña
  ...
  const handleValidateToken = async (e) => {
    ...
    try {
      const res = await fetch("/api/recuperar/validar", {
        method: "POST",
      ...
      if (res.ok && data.valid) {
        setStep(2);
        setMsg("Código válido. Ahora ingresa tu nueva contraseña.");
      } else {
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center mt-6 space-x-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
              step >= 1 ? 'bg-[#41e0b3] text-white' : 'bg-slate-200 text-slate-500'
            }`}>
              1
            </div>
            <div className={`w-8 h-1 ${step >= 2 ? 'bg-[#41e0b3]' : 'bg-slate-200'}`}></div>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
              step >= 2 ? 'bg-[#41e0b3] text-white' : 'bg-slate-200 text-slate-500'
            }`}>
              2
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
          {step === 1 ? (
            <form onSubmit={handleValidateToken} className="space-y-6">
              <div className="mb-6">
                <p className="text-slate-600 text-center leading-relaxed">
                  Ingresa tu correo electrónico y el código de 6 dígitos que recibiste.
                </p>
              </div>
            ...
            <form onSubmit={handleResetPassword} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading || newPassword !== confirmPassword || !passwordRegex.test(newPassword)}
                className="w-full bg-gradient-to-r from-[#41e0b3] to-[#2bbd8c] text-white font-semibold py-3 px-4 rounded-lg shadow-lg hover:shadow-xl hover:from-[#2bbd8c] hover:to-[#41e0b3] focus:outline-none focus:ring-2 focus:ring-[#41e0b3] focus:ring-offset-2 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed transform hover:scale-[1.02] active:scale-[0.98]"
              >
                {loading ? (
                  <span className="flex items-center justify-center">
